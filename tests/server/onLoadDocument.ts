...
test("multiple simultaneous connections wait for the document to be loaded", async (t) => {
  await new Promise(async (resolve) => {
    ...
    const p1 = new Promise<void>((resolveP1) => {
      provider1.on("synced", () => {
    ...
    const p2 = new Promise<void>((resolveP2) => {
      provider2.on("synced", () => {
    ...
    // Wait for both providers to sync, with a hard timeout to avoid hanging
    const timeout = new Promise<void>((_, reject) =>
      setTimeout(() => reject(new Error("sync timeout")), 3000)
    );
    await Promise.race([Promise.all([p1, p2]).then(() => undefined), timeout]);
